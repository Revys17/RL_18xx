...
class BaseGame:
    @classmethod
    def load(cls, data, at_action=None, actions=None, pin=None, seed=None, optional_rules=None, user=None, **kwargs):
        if isinstance(data, str):
            ...
            return cls.load(data, at_action=at_action, actions=actions, pin=pin or data.get('settings', {}).get('pin'),
                            seed=seed or data.get('settings', {}).get('seed'), optional_rules=optional_rules,
                            user=user, **kwargs)
        ...
        return game(
            names, id=id_, actions=actions, at_action=at_action, pin=pin, seed=seed, 
            optional_rules=optional_rules, user=user, **kwargs
        )
    ...
    def game_end_check(self):
        ...
        for after in ["immediate", "current_round", "current_or", "full_or", "one_more_full_or_set"]:
