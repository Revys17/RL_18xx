...
class ActionHelper():
    ...
    def get_bid_actions(self):
        ...
        bids = []
        if self.g.active_step().max_bid(self.g.current_entity, companies[0]) >= self.g.active_step().min_bid(companies[0]):
            bids.append(
                [Bid(self.g.current_entity, self.g.active_step().min_bid(companies[0]), companies[0])]
            )
        for company in companies[1:]:
            ...
            bids.append(
                [Bid(self.g.current_entity, bid_value, company) for bid_value in bid_values]
            )
    ...
    def get_company_buy_shares_actions(self, company):
        exchange_step = [step for step in self.g.round.steps if isinstance(step, ExchangeStep)][
            0
        ]
    ...
    def get_buy_train_actions(self):
        ...
        for train in trains:
            ...
            actions.append(
                [BuyTrain(self.g.current_entity, train, price) for price in price_values]
            )
    ...
    def auto_route_action(self):
        ...
        return [
            RunRoutes(
                self.g.current_entity, [route for route in best_routes if route is not None]
            )
        ]
