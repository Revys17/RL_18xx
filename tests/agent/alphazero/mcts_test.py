...
def near_terminal_game_objects(game_objects) -> GameObjects:
    ...
    # initial_game_state.process_action(action_helper.get_all_choices(initial_game_state)[0])  # [23:36] Player bankrupts
...
def test_upper_bound_confidence(game_objects, mcts_config: SelfPlayConfig):
    ...
    assert np.isclose(
        root.child_prior[0], 1.0 / num_legal_actions, rtol=1e-6
    ), f"root.child_prior[0]: {root.child_prior[0]}, should be 1/{num_legal_actions} {1.0/num_legal_actions}"
    assert np.isclose(
    ...
    assert root.N == 1
    assert np.isclose(
        root.child_U[0], puct_policy(root.N), rtol=1e-6
    ), f"root.child_U[0]: {root.child_U[0]}, should be {puct_policy(root.N)}"
    ...
    assert root.N == 1
    assert np.isclose(root.child_U[0], puct_policy(root.N) * math.sqrt(1) / (1 + 0), rtol=1e-6)
    assert np.isclose(root.child_U[1], puct_policy(root.N) * math.sqrt(1) / (1 + 0), rtol=1e-6)
    ...
    assert root.N == 3
    assert np.isclose(root.child_U[0], puct_policy(root.N) * math.sqrt(2) / (1 + 1), rtol=1e-6)
    assert np.isclose(root.child_U[1], puct_policy(root.N) * math.sqrt(2) / (1 + 1), rtol=1e-6)
    assert np.isclose(root.child_U[2], puct_policy(root.N) * math.sqrt(2) / (1 + 0), rtol=1e-6)


def test_select_leaf(game_objects, mcts_config: SelfPlayConfig):
    game_instance, action_helper, action_mapper = game_objects
...
def test_inject_noise_only_legal_moves(game_objects):
    ...
    ).all(), f"Upper bound failed. Diff: {upper_bound_check_val}, Max diff: {np.max(upper_bound_check_val)}"
