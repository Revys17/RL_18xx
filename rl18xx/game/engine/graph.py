...
__all__ = ['BasePart', 'Node', 'Path', 'Edge', 'Frame', 'FutureLabel', 'Icon', 'Junction', 'Label', 'Partition', 'Stripes',
           ...
           'DistanceGraph', 'Route', 'TileConfig', 'Tile', 'Hex']

# %% ../../../nbs/game/engine/05_graph.ipynb 4
from .core import Assignable, GameError, Ownable
...
class Route:
    def __init__(self, game, phase, train, **opts):
        ...
        self.connection_hexes = opts.get("connection_hexes")
        self.hexes = opts.get("hexes")
        self.revenue = opts.get("revenue")
        self.revenue_str = opts.get("revenue_str")
        self.subsidy = opts.get("subsidy")
        self.halts = opts.get("halts")
        self.abilities = opts.get("abilities")
        self.node_signatures = opts.get(
            "nodes"
        )  # node.signature for every node in the route
        self.local_length = game.local_length()

        self.node_chains = {}
        self.connection_data = opts.get("connection_data")
        self.last_node = None
        self.last_offboard = []
        self.stops = None
    ...
    def clear_cache(self, all=False, only_routes=False):
        if all:
            self.connection_hexes = None
            self.node_signatures = None
        self.revenue = None
        self.revenue_str = None

        if not all and only_routes:
            return

        self.ordered_paths = None
        self.ordered_hexes = None
        self.distance_str = None
        self.distance = None
        self.hexes = None
        self.paths = None
        self.stops = None
        self.subsidy = None
        self.visited_stops = None
        self.check_connected = None
    ...
    def node_signatures(self):
        if not hasattr(self, "_node_signatures"):
            chains = self.chains
    ...
    def touch_node(self, node):
        if self.connection_data and not self.local_connection():
            if node == self.head["left"]:
                ...
                if chain:
                    self.connection_data[0] = self.segment(
                ...
                if chain:
                    self.connection_data[-1] = self.segment(
            ...
            if self._train.local and len(self.connection_data) == self.local_length:
                self.connection_data.pop()
            ...
            if chain:
                ...
                if self.last_node == a:
                    a, b = b, a
                self.connection_data.append({"left": a, "right": b, "chain": chain})
        else:
            self.last_node = node
    ...
    def visited_stops(self):
        if not hasattr(self, "_visited_stops"):
    ...
    def check_cycles(self):
        ...
        for c in self.connection_data:
            right = c["right"]
            if right in cycles:
                raise RouteError(f"Cannot use {right.hex.name} twice")
            cycles[c["left"]] = True
            cycles[right] = True
    ...
    def check_connected(self):
        if not hasattr(self, "_check_connected"):
    ...
    def ordered_paths(self):
        if not hasattr(self, "_ordered_paths"):
            self._ordered_paths = [
                c["chain"]["paths"][0]
                if c["left"] in c["chain"]["paths"][0].nodes
                else c["chain"]["paths"][::-1]
                for c in self.connection_data
                if c["chain"]["paths"]
            ]
    ...
    def ordered_hexes(self):
        if not hasattr(self, "_ordered_hexes"):
    ...
    def check_terminals(self):
        ...
        if any(path.terminal for path in self.ordered_paths[1:-1]):
            raise RouteError("Route cannot pass through terminal")

    @property
    def distance_str(self):
        if not hasattr(self, "_distance_str"):
    ...
    def check_distance(self, visits):
        if not hasattr(self, "_check_distance"):
    ...
    def revenue(self, suppress_check_other=False, suppress_route_token_check=False):
        if not hasattr(self, "_revenue"):
            visited = self.visited_stops
            if (
                not self.connection_data.empty()
                and len(visited) < 2
                and not self._train.local
            ):
                raise RouteError("Route must have at least 2 stops")
            ...
            for key, group in {
                ...
                for key, group in itertools.groupby(
                    sorted(stop.groups for stop in visited)
                )
            }.items():
                if len(group) > 1:
                    raise RouteError(f"Cannot use group {key} more than once")
    ...
    @property
    def subsidy(self):
        if not hasattr(self, "_subsidy") and hasattr(self.game, "subsidy_for"):
            self._subsidy = self.game.subsidy_for(self, self.stops)
        return getattr(self, "_subsidy", None)

    @property
    def revenue_str(self):
        if not hasattr(self, "_revenue_str"):
    ...
    def connection_hexes(self):
        if not hasattr(self, "_connection_hexes"):
            if (
                self._train.local
    ...
    def connection_data(self):
        if not hasattr(self, "_connection_data"):
    ...
    def chain_id(self, paths):
        if len(paths) == 1 and paths[0].tile.ambiguous_connection():
            node0, node1 = sorted([path.nodes[0].index, path.nodes[1].index])
            ...
            for path in paths:
                ...
                junction_map[path.a] = path.a.junction()
                junction_map[path.b] = path.b.junction()
    ...
    def find_pairwise_chain(self, chains_a, chains_b, other_paths):
        ...
        if hasattr(self, "_node_signatures"):
...
class Tile:
    ...
    def for_tile(cls, name, **opts):
        color_codes = {
            ...
            **TileConfig.YELLOW,
            **TileConfig.GREEN,
            **TileConfig.GREENBROWN,
            **TileConfig.BROWN,
            **TileConfig.BROWNGRAY,
            **TileConfig.GRAY,
            **TileConfig.RED,
            **TileConfig.BLUE,
            **TileConfig.BROWNSEPIA,
        }
    ...
    def part(type, params, cache):
        if type == "path":
            ...
            return Part.Path.make_lanes(**params)

        elif type == "city":
            city = Part.City(**params)
            cache.append(city)
            return city
            ...
            pass_ = Part.Pass(
                **params
            )  # `pass` is a keyword in Python, using pass_ instead
            cache.append(pass_)
            return pass_

        elif type == "town":
            town = Part.Town(**params)
            cache.append(town)
            return town

        elif type == "halt":
            halt = Part.Halt(**params)
            cache.append(halt)
            return halt

        elif type == "offboard":
            offboard = Part.Offboard(**params)
            cache.append(offboard)
            return offboard

        elif type == "label":
            return Part.Label(**params)

        elif type == "upgrade":
            params["terrain"] = (
                params.get("terrain", "").split("|") if "terrain" in params else None
            )
            return Part.Upgrade(**params)

        elif type == "border":
            return Part.Border(**params)
            ...
            return Part.Icon(**params)

        elif type == "stub":
            return Part.Stub(edge=int(params["edge"]))

        elif type == "partition":
            return Part.Partition(**params)

        elif type == "frame":
            return Part.Frame(**params)

        elif type == "stripes":
            return Part.Stripes(**params)

        elif type == "future_label":
            return Part.FutureLabel(**params)
    ...
    def __init__(
        ...
        **opts,
        ...
        self.separate_parts()  # Assuming separate_parts is implemented
    ...
    def __lt__(self, other):
        ...
        color_order = COLORS.index(self.color)
        other_color_order = COLORS.index(other.color)
    ...
    def exit_count(self):
        ...
        if not hasattr(self, "_exit_count"):
    ...
    def __init__(self):
        self._ignore_gauge_walk = False
        self._ignore_gauge_compare = False
    ...
    def preferred_city_town_edges(self):
        if self.preferred_city_town_edges is None:
            self.preferred_city_town_edges = self.compute_city_town_edges()
        return self.preferred_city_town_edges
    ...
    def city_town_edges(self):
        ...
        return list(ct_edges.values())

    def city_town_edges_are_subset_of(self, other_cte):
        cte = self.city_town_edges()
        return any(
            all(all(edge in other_city for edge in city) for city in cte)
    ...
    def handle_special_cases(self, ct_edges, final_ct_edges):
        ...
        pathless_cts = [
            ...
            if ct.paths.empty() and len(self.cities + self.towns) == 2
        ]
    ...
    def separate_parts(self):
        for part in self.parts:
            blocks_lay = part.blocks_lay() if hasattr(part, "blocks_lay") else False
    ...
    def lay(self, tile):
        ...
        for old_city, new_city in city_map.items():
            for index, token in enumerate(old_city.tokens):
                cheater = index >= old_city.normal_slots and index
                if token:
