    ...
    def keywords(cls):
        if not hasattr(cls, "_keywords"):
            cls._keywords = (
                ...
                + [
                    (
                        Engine.Publisher.INFO[pub]["name"]
                        if pub in Engine.Publisher.INFO
                        else pub
                    )
...
# %% ../../../../nbs/game/engine/game/00_base.ipynb 8
from ..core import Train
from ..entities import Company, Corporation, Depot, Player
...
class BaseGame:
    def __init__(
        ...
        self.players = [
            self.PLAYER_CLASS(player_id, name) for player_id, name in self.names.items()
        ]
    ...
    def init_optional_rules(self, optional_rules):
        ...
        for rule in self.OPTIONAL_RULES:
    ...
    def check_optional_rules(self):
        ...
        error_string = self.meta.check_options(
            self.optional_rules, min_players, max_players
        ).get("error")
        if error_string:
            raise OptionError(error_string)
    ...
    def hex_neighbor(self, hex, edge):
        ...
        letter_index = Engine.Hex.LETTERS.index(letter)
        new_letter_index = (letter_index + d_letter) % len(Engine.Hex.LETTERS)
        new_letter = Engine.Hex.LETTERS[new_letter_index]
        number += d_number
    ...
    def register_colors(cls, colors):
        colors.default_factory = lambda key: key
        cls.COLORS = colors
    ...
    def initialize_seed(self, seed):
        ...
        if seed == "":
            seed = None
        self.seed = (
            seed or int(re.search(r"\d+", str(self.id)).group())
            if hasattr(self, "id")
            else None
        )
        self.rand = self.seed % RAND_M if self.seed else None

    def random(self):
        self.rand = ((RAND_A * self.rand) + RAND_C) % RAND_M
        return self.rand

    def inspect(self):
        player_names = [player.name for player in self.players]
        return f'{self.__class__.__name__} - {self.__class__.title} {", ".join(player_names)}'
    ...
    def active_players(self):
        players_ = [
            self.acting_for_player(entity.player)
            for entity in self.round.active_entities
            if entity and entity.player
        ]
        players_ = list(filter(None, players_))

        return (
            players_
            if players_
            else [player for player in self.players if not player.bankrupt]
        )
    ...
    def available_programmed_actions(self):
        ...
        return [Action.ProgramBuyShares, Action.ProgramSharePass]
    ...
    def initialize_actions(self, actions, at_action=None):
        ...
        if not at_action:
            self.process_to_action(actions[-1]["id"])
        else:
            self.process_to_action(at_action)

        self.redo_possible = any(active_undos)
        self.loading = False
    ...
    def process_action(
        ...
        if isinstance(action, Engine.Action.Undo) or isinstance(
            action, Engine.Action.Redo
        ):
        ...
        if not isinstance(action, Engine.Action.Message):
        ...
        if add_auto_actions or validate_auto_actions:
            ...
            if validate_auto_actions:
                if not self.auto_actions_match(action.auto_actions, auto_actions):
                    raise Engine.GameError("Auto actions do not match")
            else:
                action.clear_cache()
    ...
    def sorted_corporations(self):
        ipoed, others = partition(lambda c: c.ipoed, self.corporations)
    ...
    def buyable_bank_owned_companies(self):
        return [
            company
            for company in self.companies
            if not company.closed and company.owner == self.bank
        ]
    ...
    def all_bundles_for_corporation(self, share_holder, corporation, shares=None):
        ...
        for share in shares:
            ...
            all_bundles.append(Engine.ShareBundle(bundle[:], percent))
    ...
    def partial_bundles_for_presidents_share(self, corporation, bundle, percent):
        ...
        return [
            Engine.ShareBundle(bundle[:], percent - (normal_percent * n))
    ...
    def num_certs(self, entity):
        ...
        certs += len(entity.companies) if self.CERT_LIMIT_INCLUDES_PRIVATES else 0
        return certs
    ...
    def float_str(self, entity):
        if entity.corporation and entity.floatable:
            ...
            for stops_combination in combinations(visits, num_stops):
                ...
                for stop in stops_combination:
                    ...
                    for i, h in enumerate(distance):
                        if (
                            stop_type(stop, train) in h["nodes"]
    ...
    def abilities(
        ...
        if callable:
            for active_ability in active_abilities:
                yield active_ability, active_ability.owner
    ...
    def exchange_corporations(self, exchange_ability):
        if exchange_ability.corporations == "any":
            return [c for c in self.corporations if not c.closed]
        if exchange_ability.corporations == "ipoed":
            return [c for c in self.corporations if c.ipoed and not c.closed]
        return [
            self.corporation_by_id(c)
            for c in exchange_ability.corporations
            if not self.corporation_by_id(c).closed
        ]
    ...
    def token_owner(self, entity):
        return entity.owner if entity and entity.company else entity
    ...
    def init_tiles(self):
        return [self.init_tile(name, val) for name, val in self.game_tiles()]
    ...
    def init_company_abilities(self):
        for company in self.companies:
            ability = self.abilities(company, "shares")
            if not ability:
                continue
    ...
    def end_now(self, after):
        ...
        if after == "current_round":
            if isinstance(self.round, Round.Operating):
    ...
    def event_close_companies(self):
        ...
        for company in self.companies:
            ability = self.abilities(company, "close", on_phase="any")
            if (
                ability
    ...
    def cache_objects(self):
        for obj_type, obj_name in self.CACHABLE:
            ivar = f"@_{obj_type}"
            setattr(self, ivar, {x.id: x for x in getattr(self, obj_type)})

            def get_obj_by_id(id, ivar=ivar):
                return getattr(self, ivar).get(id)

            setattr(self, f"{obj_name}_by_id", get_obj_by_id)

    def update_cache(self, obj_type):
        if any(t == obj_type for t, _ in self.CACHABLE):
            ivar = f"@_{obj_type}"
            setattr(self, ivar, {x.id: x for x in getattr(self, obj_type)})
    ...
    def ability_right_time(self, ability, time, on_phase, passive_ok, strict_time):
        ...
        if time in ["any", ability.when["any"]]:
            return True
        ...
        if (
            ...
            and isinstance(current_step, Step.SpecialTrack)
        ):
    ...
    def ability_check_time(self, ability_when, ability, current_step, default):
        ...
        if ability_when == "owning_player_track":
            return (
                self.round.operating
                and self.round.current_operator.player == ability.player
                and isinstance(current_step, Step.Track)
            )
    ...
    def ability_usable(self, ability):
        if isinstance(ability, Ability.Token):
