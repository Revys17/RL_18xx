...
class ActionHelper:
        ...
        if self.print_enabled:
            if json_format:
                ...
                print_str += (
                    ...
                    + ",\n".join(
                        [
                            '        "' + corp + '": ' + json.dumps(info)
                            for corp, info in state["corporations"].items()
                        ]
                    )
                    + "\n    },\n"
                )
            ...
            for corp in self.g.corporations:
                ...
                print(
                    f"    share price: {corp.share_price.price if corp.share_price else None}"
                )
    ...
    def get_company_actions(self):
        company_actions = {}
        if isinstance(self.g.current_entity, Company):
            company_actions[self.g.current_entity] = self.sort_actions(
                self.g.round.actions_for(self.g.current_entity)
            )
        elif hasattr(self.g.current_entity, "companies"):
            for company in self.g.current_entity.companies:
                company_actions[company] = self.sort_actions(
                    self.g.round.actions_for(company)
                )
    ...
    def get_bid_actions(self):
        companies = self.g.active_step().companies
        bids = []
        if self.g.active_step().max_bid(
            self.g.current_entity, companies[0]
        ...
        for company in companies[1:]:
            ...
            bid_values = list(
                range(min_bid - (min_bid % 5), (max_bid + 5) - ((max_bid + 5) % 5), 5)
            )
            bids.append(
                [
        ...
        if hasattr(self.g.active_step(), "companies_pending_par"):
            ...
            parable_corporations = sorted(
                [
                    corp
                    for corp in self.g.corporations
                    if self.g.can_par(corp, self.g.current_entity)
                ],
    ...
    def get_buy_shares_actions(self):
        buyable_shares = [
            item
            for sublist in self.g.active_step().buyable_shares(self.g.current_entity)
            for item in sublist
        ]
        ...
        return sorted(
            [
                BuyShares(self.g.current_entity, share, share.price)
    ...
    def get_company_buy_shares_actions(self, company):
        exchange_step = [
            step for step in self.g.round.steps if isinstance(step, ExchangeStep)
        ][0]
    ...
    def get_sell_shares_actions(self):
        if isinstance(self.g.active_step(), BuyTrainStep):
            if (
                ...
                and self.g.current_entity.cash
                < self.g.active_step().needed_cash(self.g.current_entity)
            ):
                return [
                    SellShares(bundle.owner, bundle)
                    for bundle in self.g.active_step().sellable_shares(
                        self.g.current_entity
                    )
                ]
            else:
    ...
    def get_place_token_actions(self):
        if (
            hasattr(self.g.active_step(), "pending_token")
            and self.g.active_step().pending_token
        ):
        ...
        return sorted(
            [
                PlaceToken(
                    self.g.current_entity, city, city.get_slot(self.g.current_entity)
                )
    ...
    def get_company_place_token_actions(self, company):
        special_token_step = [
            step for step in self.g.round.steps if isinstance(step, SpecialTokenStep)
        ][0]
    ...
    def get_lay_tile_actions(self):
        ...
        for hex in self.g.graph.connected_hexes(self.g.current_entity):
            ...
            layable_tiles.extend(
                [
                    tile
                    for tile in self.g.active_step().upgradeable_tiles(
                        self.g.current_entity, hex
                    )
                    if self.g.upgrade_cost(
                        hex.tile, hex, self.g.current_entity, self.g.current_entity
                    )
            ...
            for tile in layable_tiles:
                rotations = self.g.active_step().legal_tile_rotations(
                    self.g.current_entity, hex, tile
                )
    ...
    def get_company_lay_tile_actions(self, company):
        special_track_step = [
            step for step in self.g.round.steps if isinstance(step, SpecialTrackStep)
        ][0]
        ...
        for ability in abilities:
            ...
            for hex in hexes:
                ...
                for tile in tiles:
                    rotations = special_track_step.legal_tile_rotations(
                        company, hex, tile
                    )
                    actions.extend(
                        [
        ...
        for train in trains:
            ...
            if min_price > self.g.buying_power(self.g.current_entity):
                if self.g.active_step().must_buy_train(self.g.current_entity):
                    if min_price > self.g.buying_power(
                        self.g.current_entity
                    ) + self.g.buying_power(self.g.current_entity.owner):
                        continue
                else:
                    continue
            max_price = (
                train.price if train.from_depot() else self.g.current_entity.cash
            )
            price_values = list(range(min_price, int(max_price) + 1))
            actions.append(
                [
                    BuyTrain(self.g.current_entity, train, price)
    ...
    def get_exchange_train_actions(self):
        ...
        unique_trains = self.get_unique_trains(
            [discount[1] for discount in discounted_trains]
        )
        unique_discounts = [
        ...
        return [
            BuyTrain(
                self.g.current_entity, discount[1], discount[3], exchange=discount[0]
            )
    ...
    def get_choices_for_action(self, action):
        if action == Pass:
            return [Pass(self.g.current_entity)]
        elif action == Bid:
            return self.get_bid_actions()
        elif action == Par:
    ...
    def get_all_choices(self):
        choices = [
            ...
            for choices in self.get_choices_for_action(action)
        ]
