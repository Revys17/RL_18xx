...
def get_game_object_for_game(game: dict) -> BaseGame:
    ...
    player_mapping = {player["id"]: i + 1 for i, player in enumerate(game["players"])}
...
def make_action_model_friendly(game_state: BaseGame, action: dict) -> dict:
    ...
    if action["type"] == "bid":
        ...
        if min_bid % 5 != 0:
            LOGGER.info(
                f"Minimum bid amount {min_bid} for {company.id} is not a multiple of 5. Updating to {min_bid + (5 - (min_bid % 5))}"
            )
    ...
    if action["type"] == "buy_train":
        ...
        legal_buy_train_actions = [
            x for x in all_legal_buy_train_actions if isinstance(x, BuyTrain) and x.train.id == action["train"]
        ]
        ...
        if min_diff_action is None:
            ...
            LOGGER.error(
                f"Train details: {game_state.train_by_id(action['train'])}, {game_state.train_by_id(action['train']).owner}"
            )
            LOGGER.error(
                f"Players: {game_state.corporation_by_id(action['entity']).player(), game_state.train_by_id(action['train']).owner.player()}"
            )
    ...
    if action["type"] == "buy_company":
        ...
        if (
            action["price"] == company.min_price
            or action["price"] == company.max_price
            or action["price"] == corporation.cash
        ):
...
def make_encoded_game_state_model_friendly(
...
def make_encoded_gnn_game_state_model_friendly(
    encoder: Encoder_GNN,
    encoded_game_state: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor],
    action: dict,
    game_state: BaseGame,
    ...
    if action["type"] == "bid":
        ...
        for priv_id, priv_idx in encoder.private_id_to_idx.items():
            ...
            if highest_bid.price % 5 != 0:
                LOGGER.info(
                    f"Highest bid amount {highest_bid.price} for {highest_bid.entity.id} on {company.id} is not a multiple of 5. Updating to {highest_bid.price + (5 - (highest_bid.price % 5))}"
                )
            ...
            for bid in bids:
                ...
                LOGGER.debug(
                    f"Encoding bid: Bid amount: {bid.price}, normalized: {bid_amount}, player: {bid.entity.id}, player idx: {player_idx}"
                )
        ...
        for priv_id, priv_idx in encoder.private_id_to_idx.items():
            ...
            if min_bid_val % 5 != 0:
                LOGGER.info(
                    f"Minimum bid amount {min_bid_val} for {company.id} is not a multiple of 5. Updating to {min_bid_val + (5 - (min_bid_val % 5))}"
                )
...
def do_pretraining(model_dir: str, game_data_dir: str, config: TrainingConfig) -> TrainingMetrics:
    ...
    train_dataset = HumanPlayDataset(training_data + validation_data)
