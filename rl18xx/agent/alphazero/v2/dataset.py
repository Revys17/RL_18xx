...
class MCTSDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with self.env.begin() as txn:
            data = txn.get(key)
        state, pi, value = torch.load(data)
...
class TrainingExampleProcessor:
    ...
    def write_lmdb(self, samples, lmdb_path, map_size=1e12):
        env = lmdb.open(lmdb_path, map_size=int(map_size))
        with env.begin(write=True) as txn:
            for i, (state, pi, value) in enumerate(tqdm(samples)):
                data = torch.save((state, pi, value))
                key = f'{i:08}'.encode('ascii')
                txn.put(key, data)
        LOGGER.info(f"Wrote {len(samples)} samples to {lmdb_path}")


    def make_dataset_from_selfplay(self, data_extracts):
        # Data extracts is a list of (game_state, searches_pi, result) tuples
        # We are going to convert this into a list of (encoded_state, pi, value) tuples
