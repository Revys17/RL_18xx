...
class Encoder_SSME(Encoder_1830, metaclass=Singleton):
    # --- Dynamically set in __init__ based on player count ---
    # These will be filled based on the actual game instance
    num_players: int = 0
    starting_cash: int = 0
    ...
    def _compute_base_features(self, hex_obj, game) -> np.ndarray:
        """Compute base features for a single hex (same logic as GNN version)."""
        tile = hex_obj.tile
        ...
        for i, city in enumerate(tile.cities):
            for token in city.tokens:
                if not token or not token.corporation:
                    continue
                corp_id = token.corporation.id
                if corp_id in self.corp_id_to_idx:
            ...
            if hasattr(path, 'a') and hasattr(path, 'b'):
                ...
                if len(tile.cities) > 1 or len(tile.towns) > 1:
                    if revenue_location in tile.cities:
                        idx = tile.cities.index(revenue_location)
                    elif revenue_location in tile.towns:
                        idx = tile.towns.index(revenue_location)
                    else:
                        idx = 0
                    revenue_connections[edge.num][idx] = 1.0
                else:
    ...
    def _compute_neighbor_context(self, hex_obj, hex_base_features: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        for i, neighbor in enumerate(neighbors[:6]):  # Ensure max 6 neighbors
            if neighbor.id in hex_base_features:
                ...
                # Summarize neighbor with 3 key features:
                # 1. Revenue (normalized)
                # 2. Has revenue location (city/town/offboard)
                # 3. Connectivity density (how connected internally)
