...
class Encoder_GNN(Encoder_1830, metaclass=Singleton):
    ...
    @classmethod
    def compute_section_layout(cls, num_players: int) -> Tuple[Dict[str, Tuple[int, int]], int]:
        """Compute (section_name -> (offset, size)) and total size for `num_players`.

        This is the single source of truth for the flat game-state vector layout.
    ...
    def _calculate_encoding_size(self, num_players: int) -> int:
        ...
        layout, total_size = self.compute_section_layout(num_players)
        # Maintain GAME_STATE_ENCODING_STRUCTURE shape (size_expr, size) for legacy callers.
        self.GAME_STATE_ENCODING_STRUCTURE = {
            key: (self.GAME_STATE_ENCODING_STRUCTURE[key][0], size)
            for key, (_offset, size) in layout.items()
        }
